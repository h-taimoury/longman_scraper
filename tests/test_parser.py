...
These tests never launch Playwright: the cross-reference fetch is monkeypatched
to avoid network calls, and everything else operates on static HTML.
"""
...
def test_business_entries_are_excluded(mock_fetch_cross_ref):
    mock_fetch_cross_ref.return_value = None
    html = _load_fixture()

    entries = asyncio.run(parse_word_page(html, browser=None, base_url=BASE_URL))
...
@patch("longman_scraper.parser.fetch_cross_reference_sense", new_callable=AsyncMock)
def test_basic_entry_fields(mock_fetch_cross_ref):
    mock_fetch_cross_ref.return_value = None
    html = _load_fixture()
...
@patch("longman_scraper.parser.fetch_cross_reference_sense", new_callable=AsyncMock)
def test_sense_titles_and_numbers(mock_fetch_cross_ref):
    mock_fetch_cross_ref.return_value = None
    html = _load_fixture()
    ...
    # sense 3 was a pure cross-reference and the mock returned None,
...
@patch("longman_scraper.parser.fetch_cross_reference_sense", new_callable=AsyncMock)
def test_gram_exa_and_collo_exa_examples(mock_fetch_cross_ref):
    mock_fetch_cross_ref.return_value = None
    html = _load_fixture()
...
@patch("longman_scraper.parser.fetch_cross_reference_sense", new_callable=AsyncMock)
def test_subsense_definitions_are_joined(mock_fetch_cross_ref):
    mock_fetch_cross_ref.return_value = None
    html = _load_fixture()
...
@patch("longman_scraper.parser.fetch_cross_reference_sense", new_callable=AsyncMock)
def test_cross_reference_sense_is_marked_when_resolved(mock_fetch_cross_ref):
    """When the cross-ref fetch succeeds, the sense should carry resolved data
    and be flagged as a cross-reference rather than dropped."""
    ...
    # Now sense 3 should be present and resolved.
    assert len(senses) == 5
